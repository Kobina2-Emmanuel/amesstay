...
export default function StudentTimetablePage() {
  ...
  const [profile, setProfile] = useState<{ level: string; programme: string } | null>(null)
  ...
  const fetchTimetable = async () => {
    ...
    const { data: profileData } = await supabase
      .from('profiles').select('level, programme').eq('id', user.id).maybeSingle()
    ...
    if (profileData) {
      ...
      const { data } = await supabase
        ...
        .eq('programme', profileData.programme)
        .order('start_time')
    ...
    <div>
      <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
          ...
          <div>
            <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 28, color: 'var(--text)' }}>My Timetable</h1>
            <p style={{ color: 'var(--muted)', fontFamily: 'DM Sans, sans-serif', fontSize: 13 }}>
              Level {profile?.level} · {profile?.programme === 'mated' ? 'Mathematics Education' : 'Mathematics & Economics'}
            </p>
          </div>
        </div>
      </motion.div>
          ...
          return (
            <div key={d} onClick={() => setActiveDay(d)}
              style={{
                ...
                border: active ? `1px solid ${color}` : today ? `1px solid ${color}60` : '1px solid rgba(255,255,255,0.06)',
            ...
            >
              {d}
              {today && !active && (
                <span style={{ position: 'absolute', top: -4, right: -4, width: 8, height: 8, borderRadius: '50%', background: color }} />
              )}
            </div>
          )
        })}
      ...
      {todayEntries.length === 0 ? (
        ...
        >
          ...
          <p style={{ color: 'var(--muted)', fontFamily: 'DM Sans, sans-serif', fontSize: 14 }}>
            No classes on {activeDay}. Enjoy your day! 🎉
          </p>
        </motion.div>
      ) : (
          ...
          {todayEntries.map((entry, i) => {
            ...
            return (
              <motion.div key={entry.id} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}
                style={{ background: 'var(--bg2)', border: '1px solid rgba(255,255,255,0.06)', borderLeft: `4px solid ${color}`, borderRadius: 14, padding: '20px 24px', display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}
              >
                {/* Time column */}
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 80, flexShrink: 0 }}>
                  <div style={{ width: 48, height: 48, borderRadius: 12, background: `${color}15`, border: `1px solid ${color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 6 }}>
                    <Clock size={20} color={color} />
                  </div>
                  <p style={{ fontSize: 11, color, fontFamily: 'Syne, sans-serif', fontWeight: 700, textAlign: 'center' }}>
                    {formatTime(entry.start_time)}
                  </p>
                  <p style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'DM Sans, sans-serif' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 999, background: `${color}15`, border: `1px solid ${color}30`, color, fontFamily: 'DM Sans, sans-serif' }}>
                      {entry.courses?.code}
                    </span>
                  </div>
      ...
      >
        <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 15, color: 'var(--text)', marginBottom: 16 }}>Weekly Overview</h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8 }}>
          {DAYS.map(d => {
            ...
            const today = isToday(d)
            return (
              <div key={d} onClick={() => setActiveDay(d)} style={{ textAlign: 'center', cursor: 'pointer', padding: '12px 8px', borderRadius: 10, background: activeDay === d ? `${color}15` : 'rgba(255,255,255,0.03)', border: `1px solid ${activeDay === d ? color + '40' : 'rgba(255,255,255,0.06)'}`, transition: 'all 0.2s ease' }}>
                <p style={{ fontSize: 11, color: today ? color : 'var(--muted)', fontFamily: 'DM Sans, sans-serif', marginBottom: 6 }}>{d.slice(0, 3)}</p>
                <p style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 20, color: count > 0 ? color : 'var(--muted)' }}>{count}</p>
